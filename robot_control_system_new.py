...
logging.basicConfig(
    filename='server.log',  # Log file in current working directory
    level=logging.ERROR,
    format='%(asctime)s - %(levelname)s - %(message)s'
)

# Console handler for startup messages, with filter to block HTTP requests
console = logging.StreamHandler()
console.setLevel(logging.INFO)
console.setFormatter(logging.Formatter('%(message)s'))
console.addFilter(lambda r: not ('"GET' in r.getMessage() or '"POST' in r.getMessage() or '"OPTIONS' in r.getMessage()))
...
# websiteName = 'camera.html'
websiteName = 'dark_theme_audio_slider.html'
# websiteName = 'clear_web_design.html'
...
AUTO_SELECTION_MODE = "manual"  # Options: "manual", "largest", "closest", "center", "speaking", "priority"
FACE_STABILITY_THRESHOLD = 3  # Seconds a face must be present before auto-selection
AUTO_SWITCH_COOLDOWN = 5  # Seconds before switching to another face
last_auto_switch_time = 0
face_stability_tracker = defaultdict(float)  # Track how long each face has been stable
speaking_detection_history = defaultdict(list)  # Track speaking patterns
...
def calculate_distance_from_center(face_box, screen_center):
    """Calculate distance of face center from screen center"""
    x, y, w, h = face_box
    face_center_x = x + w // 2
    face_center_y = y + h // 2
    distance = math.sqrt((face_center_x - screen_center[0]) ** 2 + (face_center_y - screen_center[1]) ** 2)
    return distance


def calculate_face_score(face_id, face_box, screen_center, current_rms=0):
    """Calculate a composite score for face selection priority"""
    area = calculate_face_area(face_box)
    distance_from_center = calculate_distance_from_center(face_box, screen_center)

    # Normalize scores (higher is better)
    area_score = min(area / 10000, 1.0)  # Normalize area
    center_score = max(0, 320 - distance_from_center) / 320  # Closer to center = higher score
    stability_score = min(face_stability_tracker[face_id] / FACE_STABILITY_THRESHOLD,
                          1.0) if face_id in face_stability_tracker else 0

    # Speaking detection bonus (if audio is active)
    speaking_score = 0
    if current_rms > 100:  # If there's significant audio
        # Give bonus to faces that have been "speaking" recently
        recent_speaking = speaking_detection_history[face_id]
        if recent_speaking:
            recent_count = sum(1 for timestamp in recent_speaking if time.time() - timestamp < 2.0)
            speaking_score = min(recent_count / 10, 1.0)
...
def auto_select_face(tracked_faces, screen_center, current_rms=0):
    """Automatically select which face to track based on the current mode"""
    global last_auto_switch_time, face_stability_tracker, speaking_detection_history, selected_face_id_for_arduino

    if not tracked_faces or AUTO_SELECTION_MODE == "manual":
    ...
    for face_id, face_data in tracked_faces.items():
        ...
        # Update speaking history if there's audio
        if current_rms > 100:
            speaking_detection_history[face_id].append(current_time)
    ...
    if current_time - last_auto_switch_time < AUTO_SWITCH_COOLDOWN:
        return None  # Keep current selection

    best_face_id = -1

    if AUTO_SELECTION_MODE == "largest":
    ...
    elif AUTO_SELECTION_MODE == "closest":
        # Select the face closest to center
    ...
    elif AUTO_SELECTION_MODE == "center":
        # Select face closest to center, but only if it's reasonably sized
        candidates = []
        for face_id, face_data in tracked_faces.items():
            area = calculate_face_area(face_data['box'])
    ...
    elif AUTO_SELECTION_MODE == "speaking":
        # Select the face most likely to be speaking (requires audio)
        if current_rms > 50:  # Only when there's audio
            best_score = 0
            for face_id, face_data in tracked_faces.items():
                # Faces closer to center are more likely to be speaking
                center_bonus = max(0, 160 - calculate_distance_from_center(face_data['box'], screen_center)) / 160
                recent_speaking = len([t for t in speaking_detection_history[face_id]
            ...
            # Fall back to largest face when no audio
            largest_area = 0
            for face_id, face_data in tracked_faces.items():
                area = calculate_face_area(face_data['box'])
                if area > largest_area:
    ...
    elif AUTO_SELECTION_MODE == "priority":
        # Use composite scoring system
        best_score = 0
        for face_id, face_data in tracked_faces.items():
            # Only consider stable faces
    ...
    # Update switch time and return selection if we're making a change
    if best_face_id != -1 and best_face_id != selected_face_id_for_arduino:
        last_auto_switch_time = current_time
        print(f"Auto-selected face {best_face_id} using {AUTO_SELECTION_MODE} mode")
        return best_face_id
...
def detect_track_and_draw_face(vid, draw_boxes_enabled):
    ...
    global current_faces_info, vision_and_eyes_active, current_rms
    ...
    with face_tracking_lock:
        ...
        auto_selected_id = auto_select_face(tracked_faces, screen_center, current_rms)
        if auto_selected_id is not None:
            selected_face_id_for_arduino = auto_selected_id
...
def generate_frames():
    ...
    while True:
        ...
        # Add status overlay
        status_text = f"Vision: {'ON' if vision_and_eyes_active else 'OFF'} | Audio: {'ON' if audio_tracking_active else 'OFF'} | Mode: {AUTO_SELECTION_MODE}"
...
@app.route('/')
def index():
    return render_template(websiteName)
...
def get_face_data():
    with face_tracking_lock:
        # Calculate sensitivity from current RMS values (inverse of the mapping in set_audio_sensitivity)
        sensitivity = (RMS_MIN_INPUT - 10) / 0.4  # Since RMS_MIN_INPUT = 10 + sensitivity * 0.4
        return jsonify({
            ...
            'auto_mode': AUTO_SELECTION_MODE,
            'rms_min_input': RMS_MIN_INPUT,
            'rms_max_input': RMS_MAX_INPUT,
    ...
    try:
        ...
        with face_tracking_lock:
            if face_id == -1 or face_id in tracked_faces:
                ...
                print(f"Manually selected face {face_id}")
...
def set_auto_mode():
    global AUTO_SELECTION_MODE, last_auto_switch_time
    try:
        data = request.get_json()
        ...
        if mode in valid_modes:
            with face_tracking_lock:
                AUTO_SELECTION_MODE = mode
                last_auto_switch_time = 0  # Reset cooldown when mode changes
            print(f"Auto-selection mode set to: {mode}")
            return jsonify({'status': 'success', 'mode': AUTO_SELECTION_MODE})
        else:
...
@app.route('/get_auto_status')
def get_auto_status():
    current_time = time.time()
    stability_scores = {}
    for face_id, start_time in face_stability_tracker.items():
        stability_scores[face_id] = current_time - start_time

    return jsonify({
        'auto_mode': AUTO_SELECTION_MODE,
        'face_stability_scores': stability_scores,
        'available_modes': ['manual', 'largest', 'closest', 'center', 'speaking', 'priority'],
        'switch_cooldown_remaining': max(0, AUTO_SWITCH_COOLDOWN - (current_time - last_auto_switch_time))
    })
...
def set_audio_sensitivity():
    global RMS_MIN_INPUT, RMS_MAX_INPUT
    try:
        data = request.get_json()
        sensitivity = float(data.get('sensitivity'))  # Expected range: 0 to 100
        # Map slider value (0-100) to RMS range
        RMS_MIN_INPUT = 10 + (sensitivity * 0.4)  # 10 to 50
        RMS_MAX_INPUT = 100 + (sensitivity * 4.0)  # 100 to 500
        print(f"Audio sensitivity set: RMS_MIN_INPUT={RMS_MIN_INPUT:.1f}, RMS_MAX_INPUT={RMS_MAX_INPUT:.1f}")
        return jsonify({
            'status': 'success',
            'rms_min_input': RMS_MIN_INPUT,
            'rms_max_input': RMS_MAX_INPUT,
...
if __name__ == '__main__':
    try:
        print("Starting Humanoid Robot Control System...")
        print("Features enabled:")
        print("  - Computer Vision Face Tracking")
        print("  - Audio-based Mouth Movement")
        print("  - Automated Face Selection")
        print("  - Web Interface Control")
        print(f"Open http://localhost:5001 or http://0.0.0.0:5001 in your browser")
